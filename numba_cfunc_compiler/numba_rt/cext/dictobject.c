...
static int ix_size(Py_ssize_t size) {
  if(size < 0xff)
    return 1;
  if(size < 0xffff)
    return 2;
...
static Py_ssize_t get_index(NB_DictKeys* dk, Py_ssize_t i) {
...
/* write to indices. */
static void set_index(NB_DictKeys* dk, Py_ssize_t i, Py_ssize_t ix) {
  Py_ssize_t s = dk->size;
...
static NB_DictEntry* get_entry(NB_DictKeys* dk, Py_ssize_t idx) {
  Py_ssize_t offset;
  char* ptr;
...
static void zero_key(NB_DictKeys* dk, char* data) { memset(data, 0, dk->key_size); }

static void zero_val(NB_DictKeys* dk, char* data) { memset(data, 0, dk->val_size); }

static void copy_key(NB_DictKeys* dk, char* dst, const char* src) { memcpy(dst, src, dk->key_size); }

static void copy_val(NB_DictKeys* dk, char* dst, const char* src) { memcpy(dst, src, dk->val_size); }

/* Returns -1 for error; 0 for not equal; 1 for equal */
static int key_equal(NB_DictKeys* dk, const char* lhs, const char* rhs) {
...
static char* entry_get_key(NB_DictKeys* dk, NB_DictEntry* entry) {
  char* out = entry->keyvalue;
  assert(out == aligned_pointer(out));
  return out;
}

static char* entry_get_val(NB_DictKeys* dk, NB_DictEntry* entry) {
  char* out = entry_get_key(dk, entry) + aligned_size(dk->key_size);
  assert(out == aligned_pointer(out));
  return out;
}

static void dk_incref_key(NB_DictKeys* dk, const char* key) {
  if(dk->methods.key_incref) {
    dk->methods.key_incref(key);
  }
}

static void dk_decref_key(NB_DictKeys* dk, const char* key) {
  if(dk->methods.key_decref) {
    dk->methods.key_decref(key);
  }
}

static void dk_incref_val(NB_DictKeys* dk, const char* val) {
  if(dk->methods.value_incref) {
    dk->methods.value_incref(val);
  }
}

static void dk_decref_val(NB_DictKeys* dk, const char* val) {
  if(dk->methods.value_decref) {
    dk->methods.value_decref(val);
  }
}

void numba_dictkeys_free(NB_DictKeys* dk) {
...
void numba_dict_free(NB_Dict* d) {
  numba_dictkeys_free(d->keys);
  free(d);
}

Py_ssize_t numba_dict_length(NB_Dict* d) { return d->used; }

/* Allocate new dictionary keys

Adapted from CPython's new_keys_object().
*/
int numba_dictkeys_new(NB_DictKeys** out, Py_ssize_t size, Py_ssize_t key_size, Py_ssize_t val_size) {
...
*/
static Py_ssize_t lookdict_index(NB_DictKeys* dk, Py_hash_t hash, Py_ssize_t index) {
...
*/
Py_ssize_t numba_dict_lookup(NB_Dict* d, const char* key_bytes, Py_hash_t hash, char* oldval_bytes) {
...
static Py_ssize_t find_empty_slot(NB_DictKeys* dk, Py_hash_t hash) {
  size_t mask;
  size_t i;
  Py_ssize_t ix;
  size_t perturb;
...
static int insertion_resize(NB_Dict* d) { return numba_dict_resize(d, D_GROWTH_RATE(d)); }

int numba_dict_insert(NB_Dict* d, const char* key_bytes, Py_hash_t hash, const char* val_bytes, char* oldval_bytes) {
...
*/
void build_indices(NB_DictKeys* keys, Py_ssize_t n) {
...
*/
int numba_dict_resize(NB_Dict* d, Py_ssize_t minsize) {
  Py_ssize_t newsize, numentries;
  NB_DictKeys* oldkeys;
  int status;

  /* Find the smallest table size > minused. */
  for(newsize = D_MINSIZE; newsize < minsize && newsize > 0; newsize <<= 1)
    ;
  if(newsize <= 0) {
    return ERR_NO_MEMORY;
  }
  ...
  status = numba_dictkeys_new(&d->keys, newsize, oldkeys->key_size, oldkeys->val_size);
  if(status != OK) {
    d->keys = oldkeys;
    return status;
  }
  ...
  if(oldkeys->nentries == numentries) {
    ...
    for(i = 0; i < numentries; ++i) {
      ...
      */
      ...
      memcpy(get_entry(d->keys, i), get_entry(oldkeys, epi), oldkeys->entry_size);
 ...
 */
int numba_dict_delitem(NB_Dict* d, Py_hash_t hash, Py_ssize_t ix) {
  Py_ssize_t hashpos;
  NB_DictEntry* ep;
  NB_DictKeys* dk = d->keys;
 ...
 *
 */
int numba_dict_popitem(NB_Dict* d, char* key_bytes, char* val_bytes) {
  Py_ssize_t i, j;
...
void numba_dict_dump(NB_Dict* d) {
  long long i, j, k;
  long long size, n;
  char* cp;
  NB_DictEntry* ep;
...
size_t numba_dict_iter_sizeof() { return sizeof(NB_DictIter); }

void numba_dict_iter(NB_DictIter* it, NB_Dict* d) {
  it->parent      = d;
...
int numba_dict_iter_next(NB_DictIter* it, const char** key_ptr, const char** val_ptr) {
  /* Detect dictionary mutation during iteration */
  NB_DictKeys* dk;
  if(it->parent->keys != it->parent_keys || it->parent->used != it->size) {
...
int numba_dict_insert_ez(NB_Dict* d, const char* key_bytes, Py_hash_t hash, const char* val_bytes) {
...
int numba_dict_new_sized(NB_Dict** out, Py_ssize_t n_keys, Py_ssize_t key_size, Py_ssize_t val_size) {
...
void numba_dict_set_method_table(NB_Dict* d, type_based_methods_table* methods) {
