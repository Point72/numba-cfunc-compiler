 ...
 * src: source pointer
 */
static void copy_item(NB_List* lp, char* dst, const char* src) { memcpy(dst, src, lp->item_size); }
 ...
 */
static void list_incref_item(NB_List* lp, const char* item) {
 ...
 */
static void list_decref_item(NB_List* lp, const char* item) {
 ...
 */
void numba_list_set_method_table(NB_List* lp, list_type_based_methods_table* methods) {
 ...
 * lp: a list
 */
void numba_list_free(NB_List* lp) {
 ...
 */
char* numba_list_base_ptr(NB_List* lp) { return lp->items; }

/* Return the address of the list size.
 */
Py_ssize_t numba_list_size_address(NB_List* lp) { return (Py_ssize_t)&lp->size; }

/* Return the length of a list.
 *
 * lp: a list
 */
Py_ssize_t numba_list_length(NB_List* lp) { return lp->size; }

/* Return the current allocation of a list.
 *
 * lp: a list
 */
Py_ssize_t numba_list_allocated(NB_List* lp) { return lp->allocated; }
 ...
 *
 */
int numba_list_is_mutable(NB_List* lp) { return lp->is_mutable; }
 ...
 *
 */
void numba_list_set_is_mutable(NB_List* lp, int is_mutable) { lp->is_mutable = is_mutable; }
 ...
 */
int numba_list_setitem(NB_List* lp, Py_ssize_t index, const char* item) {
  char* loc;
 ...
 */
int numba_list_getitem(NB_List* lp, Py_ssize_t index, char* out) {
  char* loc;
  // check index is valid
  // FIXME: this can be (and probably is) checked at the compiler level
  if(!valid_index(index, lp->size)) {
 ...
 */
int numba_list_append(NB_List* lp, const char* item) {
  char* loc;
  // check for mutability
  if(!lp->is_mutable) {
    return LIST_ERR_IMMUTABLE;
 ...
 */
int numba_list_delete_slice(NB_List* lp, Py_ssize_t start, Py_ssize_t stop, Py_ssize_t step) {
  ...
  if(step == 1) {
    ...
    for(cur = start, // index of item to be deleted
       ...
       */
      memmove(lp->items + lp->item_size * (cur - i), lp->items + lp->item_size * (cur + 1), lim * lp->item_size);
    }
    // memmove tail of the list
    cur = start + slicelength * step;
    if(cur < lp->size) {
      memmove(
        lp->items + lp->item_size * (cur - slicelength), lp->items + lp->item_size * cur,
 ...
 */
size_t numba_list_iter_sizeof() { return sizeof(NB_ListIter); }
 ...
 */
void numba_list_iter(NB_ListIter* it, NB_List* lp) {
 ...
 */
int numba_list_iter_next(NB_ListIter* it, const char** item_ptr) {
  NB_List* lp;
  lp = it->parent;
  ...
  for(i = 17; i > 0; i--) {
    switch(i) {
      ...
      case 12:
        ...
        break;
      case 9:
        CHECK(lp->allocated == 18);
        break;
      case 6:
        CHECK(lp->allocated == 12);
        break;
      case 4:
        CHECK(lp->allocated == 8);
        break;
      case 3:
        CHECK(lp->allocated == 6);
        break;
      case 2:
        CHECK(lp->allocated == 5);
        break;
      case 1:
        CHECK(lp->allocated == 4);
        break;
