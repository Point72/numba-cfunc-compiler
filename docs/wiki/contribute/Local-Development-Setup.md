...
To work on `numba-cfunc-compiler`, you are going to need to build it from source. See
[Build from Source](Build-from-Source) for
detailed build instructions.

Once you've built `numba-cfunc-compiler` from a `git` clone, you will also need to
configure `git` and your GitHub account for `numba-cfunc-compiler` development.

## Step 2: Configuring Git and GitHub for Development

### Create your fork

The first step is to create a personal fork of `numba-cfunc-compiler`. To do so, click
the "fork" button at https://github.com/Point72/numba-cfunc-compiler, or just navigate
[here](https://github.com/Point72/numba-cfunc-compiler/fork) in your browser. Set the
